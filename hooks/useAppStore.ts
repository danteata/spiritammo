...
import { Book, Collection, Scripture, UserSettings, UserStats } from '@/types/scripture';
...
export const [AppStoreProvider, useAppStore] = createContextHook(() => {
  ...
  const updateUserStats = async (accuracy: number) => {
    try {
      ...
      const lastPracticeDate = userStats.lastPracticeDate ? new Date(userStats.lastPracticeDate).toDateString() : null;
  ...
  const getScripturesByCollection = (collectionId: string) => {
    ...
    return scriptures.filter(scripture => collection.scriptures.includes(scripture.id));
  };
  ...
  const getScripturesByBookAndChapter = (bookId: string, chapter: number) => {
    return scriptures.filter(
      scripture => scripture.book.toLowerCase() === bookId.toLowerCase() && scripture.chapter === chapter
    );
  };
  ...
  const getRandomScripture = () => {
    ...
    if (selectedBook) {
      ...
      if (selectedChapters.length > 0) {
        filteredScriptures = filteredScriptures.filter(
          s => selectedChapters.includes(s.chapter)
        );
      }
    }
  ...
  const updateScriptureAccuracy = async (scriptureId: string, accuracy: number) => {
    try {
      ...
      if (currentScripture?.id === scriptureId) {
        setCurrentScripture(updatedScriptures.find(s => s.id === scriptureId) || null);
      }

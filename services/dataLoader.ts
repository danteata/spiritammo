...
export class DataLoaderService {
  ...
  static async loadTransformedData(): Promise<{
    ...
    try {
      ...
      const mergedScriptures = this.mergeScriptures(
        existingScriptures,
        transformedScriptures
      )
      ...
      await AsyncStorage.setItem(
        SCRIPTURES_KEY,
        JSON.stringify(mergedScriptures)
      )
      await AsyncStorage.setItem(DATA_LOADED_KEY, 'true')

      console.log(
        `Loaded ${transformedCollections.length} collections and ${transformedScriptures.length} scriptures`
      )
  ...
  private static mergeCollections(
    existing: Collection[],
  ...
  private static mergeScriptures(
    existing: Scripture[],
  ...
  static async forceReloadData(): Promise<{
    ...
    try {
      // Clear the loaded flag and all existing data
      ...
      // Reload data with new transformation
  ...
  // Force update to include Words of Jesus (call this once to update existing data)
  static async updateWithWordsOfJesus(): Promise<boolean> {
    try {
      console.log('🔄 Updating scriptures with Words of Jesus highlighting...')

      // Clear existing data to force re-transformation
      await AsyncStorage.multiRemove([
        DATA_LOADED_KEY,
        COLLECTIONS_KEY,
        SCRIPTURES_KEY,
      ...
      // Load fresh data with Words of Jesus markers
      const result = await this.loadTransformedData()

      if (result.success) {
        console.log('✅ Words of Jesus update complete!')
        return true
      } else {
      ...
      console.error('Error updating with Words of Jesus:', error)
      return false
    }
  }
  ...
  static getCollectionStats(collections: Collection[]): {
    ...
    const totalChapters = chapterBased.reduce(
      (sum, c) => sum + (c.chapters?.length || 0),
      0
    )
    const totalScriptures = collections.reduce(
      (sum, c) => sum + c.scriptures.length,
      0
    )

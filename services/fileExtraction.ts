...
class FileExtractionService {
  ...
  private async extractVerses(
    ...
    for (const pattern of versePatterns) {
      ...
      for (const match of matches) {
        ...
        if (cleanText.length > 10 && cleanText.length < 500) { // Reasonable verse length
  ...
  private calculateConfidence(text: string, book?: string): number {
    ...
    const biblicalWords = [
      'lord', 'god', 'jesus', 'christ', 'holy', 'spirit', 'father', 'heaven',
      'blessed', 'righteousness', 'salvation', 'faith', 'love', 'peace', 'joy',
      'prayer', 'worship', 'praise', 'glory', 'amen', 'hallelujah'
    ];

    const lowerText = text.toLowerCase();
    const biblicalWordCount = biblicalWords.filter(word => lowerText.includes(word)).length;
  ...
  private looksLikeVerse(text: string): boolean {
    ...
    return (
      ...
      (trimmed.includes('Lord') || trimmed.includes('God') || trimmed.includes('Jesus'))
    );
  }
  ...
  private isKnownBibleBook(book: string): boolean {
    ...
    return bibleBooks.some(b => b.toLowerCase().includes(normalizedBook) || normalizedBook.includes(b.toLowerCase()));
  }
